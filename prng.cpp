...
/************************************************************************/
/* Return values that are 4-wise independent                            */
/************************************************************************/
/*
...
/************************************************************************/
/* Pseudo-random number generators using Numerical Recipes gens         */
/************************************************************************/
...
void RanrotAInit (prng_type * prng, unsigned long seed)
{
  ...
  for (i = 0;  i < 300;  i++)
	  ran3(prng);
  prng->scale = ldexp(1.0, -8*sizeof(unsigned long));
}
...
long prng_int(prng_type * prng)
{
  ...
  switch (prng->usenric)
	...
	// lrand48() does not exist in stdlib.h of C++
	// Can be replaced by (double(rand()) / RAND_MAX)
...
float prng_float(prng_type * prng)
{
  ...
  switch (prng->usenric)
	...
	// drand48() does not exist in stdlib.h of C++
	// Can be replaced by (double(rand()) / RAND_MAX)
...
prng_type * prng_Init(long seed, int nric)
{
  ...
  switch (nric)
    ...
    case 1 :
      ...
      break;
    case 3 :
	// srand48(seed) does not exist in stdlib.h of C++
	// Can be replaced by srand(seed)
    // srand48(seed);
      break;
    }
...
double prng_stable(prng_type * prng, double alpha)
{
  ...
  if (alpha == 2.0)
	...
	return (prng_stabledbn(prng, alpha));
}
